...
(function () {
  ...
  function rezka2(component, _object) {
    ...
    var ref = host + '/';
    var logged_in = !(prox || Lampa.Platform.is('android') || isAppleTV);
    var user_agent = Utils.baseUserAgent();
    var headers = (Lampa.Platform.is('android') || isAppleTV) ? {
      'Origin': host,
      'Referer': ref,
      'User-Agent': user_agent
    } : {};
    ...
    if (cookie) {
      if (Lampa.Platform.is('android') || isAppleTV) {
    ...
    this.search = function (_object, kinopoisk_id, data) {
      ...
      var query_more = function query_more(query, page, data, callback) {
        ...
        network["native"](component.proxyLink(url, prox, prox_enc, prox_enc), function (str) {
          str = (str || '').replace(/\n/g, '');
          checkErrorForm(str);
          var links = str.match(/<div class="b-content__inline_item-link">\s*<a [^>]*>[\s\S]*?<\/a>\s*<div>[\s\S]*?<\/div>\s*<\/div>/g);
  ...
  var isAppleTV = /AppleTV|tvOS/i.test(navigator.userAgent) ||
    (navigator.platform === 'MacIntel' && navigator.maxTouchPoints === 0 && screen.width >= 1920) ||
    (/iPad/i.test(navigator.userAgent) && (screen.width >= 1920 || screen.height >= 1920));
  ...
  function logApp() {
    ...
    console.log('App', 'is AppleTV:', isAppleTV);
  ...
  function initStorage() {
    if (!Utils.isDebug() && !isAppleTV) {
      ...
      // Apple TV: включаем прокси для HDRezka по умолчанию
      if (isAppleTV) {
      ...
      if (e.name == 'filmix') {
        e.body.find('[data-name="filmix_add"]').unbind('hover:enter').on('hover:enter', function () {
          ...
          network["native"](Utils.proxyLink(api_url + 'token_request' + Utils.filmixToken(dev_id, ''), filmix_prox, filmix_prox_enc, 'enc2'), function (found) {
            ...
            modal.find('.broadcast__scan').remove();
            Lampa.Noty.show('Rezka Login: ' + network.errorDecode(a, c) + (a.responseText ? ' | ' + a.responseText.substring(0, 50) : ''));
          }, false, {
            headers: filmix_headers
          });
        });
  ...
  function rezka2FillCookie(success, error) {
    ...
    var prox_enc = '';
    var returnHeaders = androidHeaders || isAppleTV;
    var proxy_mirror = Lampa.Storage.field('online_mod_proxy_rezka2_mirror') === true;
    var host = prox && !proxy_mirror ? 'https://rezka.ag' : Utils.rezka2Mirror();
  ...
  function initSettings() {
    ...
    if (Utils.isDebug() || Lampa.Platform.is('android') || isAppleTV) {
  ...
  function startPlugin() {
    ...
    logApp();
    // DEBUG: показать уведомление о платформе (можно удалить после отладки)
    Lampa.Noty.show('AppleTV: ' + isAppleTV + ' | UA: ' + navigator.userAgent.substring(0, 50));
    initStorage();
    initLang();
    initMain();
    initFilmix();
